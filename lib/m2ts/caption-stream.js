...
var CaptionStream = function(options) {
  ...
  if (this.parse708captions_) {
    this.cc708Stream_ = new Cea708Stream({serviceEncodings: options.captionServiceEncodings}); // eslint-disable-line no-use-before-define
  }
...
var Cea708Service = function(serviceNum, encoding) {
...
Cea708Service.prototype.createTextDecoder = function(encoding) {
  if (typeof TextDecoder === 'undefined') {
    this.trigger('log', {
      level: 'warn',
      message: 'The `encoding` option is unsupported without native TextDecoder support'
    });
  } else {
    try {
      ...
      this.trigger('log', {
        level: 'warn',
        message: 'TextDecoder could not be created for provided `encoding` value: ' + encoding
      });
    }
  }
}
...
var Cea708Stream = function(options) {
  ...
  this.serviceEncodings = options.serviceEncodings || {};
...
Cea708Stream.prototype.initService = function(serviceNum, i) {
  var self = this;
  var serviceNames = Object.keys(this.serviceEncodings);
  var serviceName;
  var providedEncoding;
  var encoding;

  // Check if an encoding value was specified for this service
  for (var i = 0; i < serviceNames.length; i++) {
    serviceName = serviceNames[i];
    providedEncoding = this.serviceEncodings[serviceName];

    if (serviceName === 'SERVICE' + serviceNum) {
      encoding = providedEncoding;
      break;
    }
  }

  this.services[serviceNum] = new Cea708Service(serviceNum, encoding);
...
Cea708Stream.prototype.handleText = function(i, service, isExtended) {
  ...
  var previousByte = packetData[i - 1];
  ...
  if (service.textDecoder_) {
    // If preceded by a code in the 0x18-0x1f range, this byte and the following byte make up a two-byte character
    if (previousByte >= 0x18 && previousByte <= 0x1f && nextByte) {
      charCodeArray = [currentByte, nextByte];

      // Skip the next byte in the loop since it has already been handled
      i++;
    } else {
