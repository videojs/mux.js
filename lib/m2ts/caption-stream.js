...
var Cea608Stream = function(field, dataChannel) {
  ...
  this.push = function(packet) {
    ...
    // Detect offset control codes and adjust cursor
      ...
      // For the offest values 1-3, add a character to the beginning
      // of that caption text that amount of times.
    ...
    // Detect PACs (Preamble Address Codes)
      ...
      if ((data & 0x10) === 0x10) {
        ...
        // // Add 4 blank spaces for evey indentation to the caption text.
...
Cea608Stream.prototype.flushDisplayed = function(pts) {
  ...
  this.displayed_.forEach((row, i) => {
    if (row && row.length) {
      ...
      // Before trimming the whitespace, add to the indent count
      // until we find a non-empty character.
      let indent = 0;
      while(EMPTY_SPACE.test(row[indent])) {
        indent++;
      }
      ...
      if (row.length) {
        content.push({
          ...
          // The value will be a number between 10 and 80.
          position: 10 + Math.min(70, Math.floor((indent * 8) / 32) * 10),
        });
      }
    }
