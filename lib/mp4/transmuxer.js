...
AudioSegmentStream = function(track) {
  ...
  this.generateSampleTable_ = function(frames, track) {
    var
      i,
      currentFrame,
      silence,
    ...
    for (i = 0; i < frames.length; i++) {
      ...
      silence = coneOfSilence[currentFrame.samplerate];

      if (i > 0) {
        duration = Math.round((currentFrame.pts - frames[i-1].pts) * currentFrame.samplerate / 90000);
      } else {
        duration = 1024;
      }

      if (duration >= 1023 && duration <= 1025) {
        duration = 1024;
      }

      // Silence insertion for missing AAC frames
      while (duration > 1024) {
        samples.push({
          size: silence.byteLength,
