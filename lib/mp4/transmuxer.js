...
VideoSegmentStream = function(track) {
  ...
  this.flush = function() {
    var startUnit, currentNal, moof, mdat, boxes, i, data, view, sample, hasPPS, hasSPS;
    ...
    while (nalUnits.length) {
      ...
      if (i > 0 && currentNal.nalUnitType === 'access_unit_delimiter_rbsp') {
        nalUnits.splice(1, 0, {
          data: track.sps[0],
          type: 'seq_parameter_set_rbsp'
        });
        nalUnitsLength += track.sps[0].byteLength;
        nalUnits.splice(2, 0, {
          data: track.pps[0],
          type: 'pic_parameter_set_rbsp'
        });
        nalUnitsLength += track.pps[0].byteLength;
        break;
      }
      if (currentNal.nalUnitType === 'seq_parameter_set_rbsp') {
        break;
      }
      if (currentNal.nalUnitType === 'pic_parameter_set_rbsp') {
        break;
      }
      i++;
    }
    ...
    while (nalUnits.length) {
      ...
      if (currentNal.nalUnitType === 'access_unit_delimiter_rbsp') {
        if (startUnit) {
          sample.duration = currentNal.dts - startUnit.dts + 1;
          track.samples.push(sample);
        }
        sample = {
          size: 0,
