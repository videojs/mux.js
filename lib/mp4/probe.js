...
var findBox, parseType, timescale, startTime, findSeiNals,
  parseCaptionNals, parseInitForCaptionMetadata, parseSegmentForSamples, parseEmbeddedCaptions;
var parseHandlerType, parseTrackId, parseMdhd;
...
parseCaptionNals = function(fragment, trafSamples) {
  var captionNals = [];
  var mdats = findBox(fragment, ['mdat']);

  for (var i = 0; i < mdats.length; i++) {
    // mdat and moof pair
    var mdat = mdats[i];
    var samples = trafSamples[i].samples;
    var trackId = trafSamples[i].trackId;
...
parseSegmentForSamples = function(segment, timescale) {
...
parseEmbeddedCaptions = function(init, segment) {
  ...
  for (var trackId in initInfo) {
    ...
    var samples = parseSegmentForSamples(segment, timescale);
    var seiNals = parseCaptionNals(segment, samples);
