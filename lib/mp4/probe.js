    ...
    if (stsd) {
      ...
      if (codecBox) {
        ...
        if ((/^[asm]vc[1-9]$/i).test(track.codec)) {
          ...
          // Just hvc1 or hev1 as codec string is not sufficient, canPlayType won't accept it
          ...
          if (codecConfigType === 'hvcC' && codecConfig.length > 21) {
            ...
            // Trailing zeros removed from joined string. Although the spec says trailing zeros
            // may be omitted, every example has one byte included even if zero.
            track.codec = [
              track.codec,
