...
var mapToSample = function(offset, samples) {
  ...
  if (offset > lastSample.dataOffset) {
    return lastSample;
  }
  ...
  truns.forEach(function(trun) {
    ...
    samples.forEach(function(sample) {
      if (sample.duration === undefined) {
        sample.duration = defaultSampleDuration || 0;
      }
      if (sample.size === undefined) {
        sample.size = defaultSampleSize || 0;
      }
...
var parseEmbeddedCaptions = function(init, segment) {
  ...
  // Use the first video track only to match M2TS handling
  trackId = videoTrackIds[0];

  trackTimescale = timescales[trackId];
  trackSeiNals = seiNals[trackId];
  ...
  captionStream.on('data', function(event) {
    ...
    captionStreams[event.stream] = true;
  });
