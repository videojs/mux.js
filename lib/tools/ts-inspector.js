  ...
  while (endIndex < bytes.byteLength) {
    ...
    if (bytes[startIndex] === SYNC_BYTE && bytes[endIndex] === SYNC_BYTE) {
      ...
      if (probe.ts.parseType(packet, pmt.pid) === 'pes' &&
          probe.ts.parsePesType(packet, pmt.table)) {
        return {
          packet: packet,
...
var getFullAdtsHeader_ = function(bytes, pmt, packet, startIndex, endIndex) {
  ...
  for (i = 0; i < packetsAvailable; i++) {
    headerOffset = probe.ts.getAdtsHeaderOffset(packet);

    if (headerOffset) {
      // header start found, get as much of the header as possible
      headerBytes = headerBytes ?
        typedArrayUtils.concatTypedArrays(headerBytes, packet.subarray(headerOffset)) :
        packet.subarray(headerOffset);
    }
    ...
    if (headerBytes && headerBytes.byteLength >= 2) {
      ...
      if (headerBytes.byteLength >= MIN_ADTS_HEADER_BYTES + (crcPresent ? 2 : 0)) {
        return headerBytes;
      }
    }
...
var parseFrameDuration_ = function(bytes, pmt, packet, startIndex, endIndex) {
  ...
  return parsers.parseAdtsHeader(adtsHeader, 0).adtsFrameDuration;
};
