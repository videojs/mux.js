...
var getFullAdtsHeaderFromElementaryStream_ = function(bytes) {
  var
    crcPresent,
    headerBytes,
    headerLength,
    headerOffset = probe.ts.getAdtsHeaderOffset(bytes);

  if (headerOffset !== null) {
    // header start found, get as much of the header as possible
    headerBytes = bytes.subarray(headerOffset);
  }
...
var getFullAdtsHeader_ = function(bytes, pmt, packet, startIndex, endIndex) {
  var
    ...
    pesHeaderLengthIndex,
    packetsAvailable = (bytes.byteLength - startIndex - 1) / MP2T_PACKET_LENGTH;

  for (i = 0; i < packetsAvailable; i++) {
    adaptationFieldLength = probe.ts.parseAdaptionField(packet);

    if (adaptationFieldLength >= packet.byteLength) {
      // packet is just adaptation field stuffing bytes (no data), so move to the next one
    ...
    // account for the TS header and adaptation field length to start
    esOffset = 4 + adaptationFieldLength;
    pusi = probe.ts.parsePayloadUnitStartIndicator(packet);

    if (pusi) {
      // A PES packet will only begin (i.e., provide a header) if the PUSI is set to 1.
      // And if a PES packet begins, we have to offset our reading of the elementary
      // stream bytes by the header length.

      // Since we know it is an audio PES packet, we can bypass the 4 byte start code, 2
      // byte PES packet length, and the first 2 optional PES header bytes to get to the
      // PES header length (remainder) byte.
      pesHeaderLengthIndex = esOffset + 4 + 2 + 2;
      pesHeaderLength = packet[pesHeaderLengthIndex];
      // add 1 to move to start of elementary stream
      esOffset = pesHeaderLengthIndex + 1 + pesHeaderLength;
    }

    packetEsBytes = packet.subarray(esOffset, endIndex);
    esBytes = esBytes ?
      typedArrayUtils.concatTypedArrays(esBytes, packetEsBytes) : packetEsBytes;
    header = getFullAdtsHeaderFromElementaryStream_(esBytes);
