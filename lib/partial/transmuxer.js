...
var _audioSegmentStream = _interopRequireDefault(require("./audio-segment-stream.js"));

var _videoSegmentStream = _interopRequireDefault(require("./video-segment-stream.js"));

var _trackDecodeInfo = require("../mp4/track-decode-info.js");

var _utils = require("../aac/utils");

var _index = _interopRequireDefault(require("../aac/index"));
...
var createPipeline = function createPipeline(object) {
  object.prototype = new _stream.default();
...
var tsPipeline = function tsPipeline(options) {
  var pipeline = {
    ...
    packet: new _m2ts.TransportPacketStream(),
    parse: new _m2ts.TransportParseStream(),
    elementary: new _m2ts.ElementaryStream(),
    videoRollover: new _timestampRolloverStream.TimestampRolloverStream('video'),
    audioRollover: new _timestampRolloverStream.TimestampRolloverStream('audio'),
    adts: new _adts.default(),
    h264: new _h.H264Stream(),
    captionStream: new _captionStream.CaptionStream(),
    metadataStream: new _metadataStream.default(),
    timedMetadataRollover: new _timestampRolloverStream.TimestampRolloverStream('timed-metadata')
  };
  ...
  pipeline.elementary.on('data', function (data) {
    ...
    if (pipeline.tracks.video && !pipeline.videoSegmentStream) {
      pipeline.videoSegmentStream = new _videoSegmentStream.default(pipeline.tracks.video, options);
    ...
    if (pipeline.tracks.audio && !pipeline.audioSegmentStream) {
      pipeline.audioSegmentStream = new _audioSegmentStream.default(pipeline.tracks.audio, options);
  ...
  pipeline.captionStream.on('data', function (caption) {
    ...
    caption.startTime = (0, _clock.metadataTsToSeconds)(caption.startPts, timelineStartPts, options.keepOriginalTimestamps);
    caption.endTime = (0, _clock.metadataTsToSeconds)(caption.endPts, timelineStartPts, options.keepOriginalTimestamps);
...
var aacPipeline = function aacPipeline(options) {
  var pipeline = {
    ...
    metadataStream: new _metadataStream.default(),
    aacStream: new _index.default(),
    audioRollover: new _timestampRolloverStream.TimestampRolloverStream('audio'),
    timedMetadataRollover: new _timestampRolloverStream.TimestampRolloverStream('timed-metadata'),
    adtsStream: new _adts.default(true)
...
var setupPipelineListeners = function setupPipelineListeners(pipeline, transmuxer) {
  ...
  pipeline.on('id3Frame', function (event) {
    ...
    event.cueTime = (0, _clock.videoTsToSeconds)(event.pts);
...
var Transmuxer = function Transmuxer(options) {
  ...
  this.push = function (bytes) {
    if (hasFlushed) {
      var isAac = (0, _utils.isLikelyAacData)(bytes);
