...
var _captionStream = _interopRequireDefault(require("../m2ts/caption-stream.js"));
...
var _coalesceStream = _interopRequireDefault(require("./coalesce-stream.js"));
...
metaDataTag = function metaDataTag(track, pts) {
  var tag = new _flvTag.default(_flvTag.default.METADATA_TAG); // :FlvTag
...
extraDataTag = function extraDataTag(track, pts) {
  var i,
      tag = new _flvTag.default(_flvTag.default.VIDEO_TAG, true);
  tag.dts = pts;
  tag.pts = pts;
...
_AudioSegmentStream = function AudioSegmentStream(track) {
  ...
  this.flush = function () {
    var currentFrame,
        adtsFrame,
        lastMetaPts,
        tags = new _tagList.default(); // return early if no audio data has been observed
    ...
    while (adtsFrames.length) {
      ...
      adtsFrame = new _flvTag.default(_flvTag.default.AUDIO_TAG);
  ...
  this.writeMetaDataTags = function (tags, pts) {
    var adtsFrame;
    adtsFrame = new _flvTag.default(_flvTag.default.METADATA_TAG); // For audio, DTS is always the same as PTS. We want to set the DTS
    ...
    tags.push(adtsFrame.finalize());
    adtsFrame = new _flvTag.default(_flvTag.default.AUDIO_TAG, true); // For audio, DTS is always the same as PTS. We want to set the DTS
    // however so we can compare with video DTS to determine approximate
    // packet order
...
_AudioSegmentStream.prototype = new _stream.default();
/**
...
_VideoSegmentStream = function VideoSegmentStream(track) {
  ...
  this.flush = function () {
    var currentNal,
        tags = new _tagList.default(); // Throw away nalUnits at the start of the byte stream until we find
    // the first AUD
    ...
    while (nalUnits.length) {
      ...
      if (currentNal.nalUnitType === 'seq_parameter_set_rbsp') {
        ...
        h264Frame = new _flvTag.default(_flvTag.default.VIDEO_TAG);
...
_VideoSegmentStream.prototype = new _stream.default();
/**
...
_Transmuxer = function Transmuxer(options) {
  ...
  this.metadataStream = new _metadataStream.default();
  options.metadataStream = this.metadataStream; // set up the parsing pipeline

  packetStream = new _m2ts.TransportPacketStream();
  parseStream = new _m2ts.TransportParseStream();
  elementaryStream = new _m2ts.ElementaryStream();
  videoTimestampRolloverStream = new _timestampRolloverStream.TimestampRolloverStream('video');
  audioTimestampRolloverStream = new _timestampRolloverStream.TimestampRolloverStream('audio');
  timedMetadataTimestampRolloverStream = new _timestampRolloverStream.TimestampRolloverStream('timed-metadata');
  adtsStream = new _adts.default();
  h264Stream = new _h.H264Stream();
  coalesceStream = new _coalesceStream.default(options); // disassemble MPEG2-TS packets into elementary streams
  ...
  captionStream = new _captionStream.default();
...
_Transmuxer.prototype = new _stream.default(); // forward compatibility

var _default = _Transmuxer;
